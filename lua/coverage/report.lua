...
local get_cov_hl_group = function(threshold)
    local min_threshold = config.opts.report.min_coverage
...
local load_header = function()
    ...
    table.insert(
        header.highlights,
        { hl_group = "CoverageReportHeader", line = #header.lines, col_start = 0, col_end = -1 }
    )
    table.insert(
        header.lines,
        string.format(
...
local render_help = function()
    ...
    vim.api.nvim_buf_set_lines(popup.bufnr, 0, -1, false, lines)
    set_modifiable(false)
    add_highlight({ hl_group = "CoverageReportHeader", line = 0, col_start = 0, col_end = -1 })
    help_displayed = true
end
...
M.show = function()
    ...
    local win_opts = vim.tbl_deep_extend("force", {}, config.opts.report.window)

    popup = window.percentage_range_window(
        adjust_width_percentage(config.opts.report.width),
        config.opts.report.height,
        win_opts,
        border_opts
    )
