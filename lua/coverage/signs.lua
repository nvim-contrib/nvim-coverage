...
--- Defines signs.
M.setup = function()
    vim.fn.sign_define(name("covered"),  { text = config.opts.signs.covered.text,  texthl = config.opts.signs.covered.hl })
    vim.fn.sign_define(name("uncovered"), { text = config.opts.signs.uncovered.text, texthl = config.opts.signs.uncovered.hl })
    vim.fn.sign_define(name("partial"),  { text = config.opts.signs.partial.text,  texthl = config.opts.signs.partial.hl })
end
...
M.show = function()
    if enabled or cached_signs == nil then return end
...
M.jump = function(sign_type, direction)
    if not enabled or cached_signs == nil then return end
    ...
    if direction < 0 then
        table.sort(placed_signs, function(a, b) return a.lnum > b.lnum end)
    end
...
M.new_covered = function(buffer, lnum)
    return { buffer = buffer, group = config.opts.sign_group, lnum = lnum,
             name = name("covered"), priority = config.opts.signs.covered.priority or default_priority }
end
...
M.new_uncovered = function(buffer, lnum)
    return { buffer = buffer, group = config.opts.sign_group, lnum = lnum,
             name = name("uncovered"), priority = config.opts.signs.uncovered.priority or default_priority }
end
...
M.new_partial = function(buffer, lnum)
    ...
    return { buffer = buffer, group = config.opts.sign_group, lnum = lnum,
             name = name("partial"), priority = priority }
end
